...
public class DiscardStockCommandHandlerTests
{
    ...
    private DiscardStockCommandHandler BuildHandler(List<StockLot> stockLots)
    {
        ...
        return new DiscardStockCommandHandler(_currentUser, _context, new StockService(_context, _currentUser));
    }
    ...
    [Fact]
    public async Task Reduces_the_lot_and_records_discarded()
    {
        var lot = Lot(10);
        var handler = BuildHandler([lot]);

        var result = await handler.Handle(
            new DiscardStockCommand(_lotId, 4, "spoiled"), CancellationToken.None);
    ...
    public async Task Removes_the_lot_when_it_reaches_zero()
    {
        ...
        var result = await handler.Handle(
            new DiscardStockCommand(_lotId, 4, null), CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        _context.StockLots.Received(1).Remove(lot);
        _context.Movements.Received(1).Add(Arg.Is<Movement>(m => m.Type == MovementType.Discarded && m.Quantity == 4));
    }
    ...
    public async Task Fails_when_the_lot_does_not_exist_in_the_household()
    {
        ...
        var result = await handler.Handle(
            new DiscardStockCommand(_lotId, 1, null), CancellationToken.None);
